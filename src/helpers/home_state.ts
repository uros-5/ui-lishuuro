...
const timeSchema = 
  v.pipe(v.number(), v.minValue(1), v.maxValue(allowedDuration.length))
...
export function homeState() {
  ...
  let subVariant = v.safeParse(v.pipe(v.number(), v.minValue(0), v.maxValue(101)), toNumber("subvariant"))
...
export function updateState(minutes: number, increment: number, variant: Description | undefined , board: number, ) {
