<template>
  <div id="id01" class="modal" style="display: block" v-if="store.$state.clickedVariant != ''">
    <form class="modal-content">
      ...
      <div class="container">
        ...
        <div id="color-button-group" style="display: block;">
            <button @click="color = 'black'; createGame();" class="icon icon-black" type="button" title="Black"></button>
            <button @click="color = 'random'; createGame();" class="icon icon-adjust" type="button" title="Random"></button>
            <button @click="color = 'white'; createGame();" class="icon icon-white" type="button" title="White"></button>
        </div>
      </div>
    </form>
  </div>
...
function createGame() {
    let game = { t: "home_lobby_add", time: time.value, incr: incr.value, variant: variant.value, color: color.value, username: userStore.$state.username};
