...
<div class="pocket-top">
    <div class="standard-placement">
        <div class="cg-wrap pocket">
            <div :id="pID" class="pocket top usable" :class="part" :style="`--pocketLength: 5; --files: ${store.$state.rowsAndCols}; --ranks: ${store.$state.rowsAndCols}; ${store.genVars()}`">
                <piece v-for="i in destructHand()" :key="i" :class="genClass(color, i.p)" :data-nb="i.nb"></piece>
            </div>
        </div>
    </div>
</div>
...
const props = defineProps({pieces: String, color: String, pID: String, part: String});
...
function destructHand(): String {
    let hand = [{p: 'q', nb: 0},{p: 'r',nb: 0},{p: 'b',nb: 0},{p: 'n',nb: 0},{p: 'p',nb: 0}];
    for (let i = 0;i < props.pieces.length; i++) {
        let p = props.pieces[i];
        switch (p) {
            case 'q': hand[0].nb += 1;break;
            case 'r': hand[1].nb += 1;break;
            case 'b': hand[2].nb += 1;break;
            case 'n': hand[3].nb += 1;break;
            case 'p': hand[4].nb += 1;break;
        }
    }
    return hand;
}
