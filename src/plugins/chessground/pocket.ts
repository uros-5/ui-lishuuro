...
function renderPiece(el: HTMLElement, state: HeadlessState) {
  ...
  const selectedSquare =
    dropMode.active && dropPiece?.role === role && dropPiece.color === color;
...
export function renderPocketsInitial(
  ...
  function pocketView(pocketEl: HTMLElement, position: cg.PocketPosition) {
    ...
    pocketEl.setAttribute(
      "style",
      `--pocketLength: ${pl}; --files: ${files}; --ranks: ${ranks}`
    );
    pocketEl.classList.add("pocket", position, "usable");

    roles.forEach((role: string) => {
      const pieceName = util.pieceClasses(
        { role: role, color: color, promoted: false } as cg.Piece,
        state.orientation
      );
...
export function click(state: HeadlessState, e: cg.MouchEvent): void {
  ...
  if (
    (!dropMode.active || dropPiece?.role !== role) &&
    canceledDropMode !== "true"
  ) {
...
export function drag(state: HeadlessState, e: cg.MouchEvent): void {
  ...
  if (state.movable.dests) {
    const dropDests = new Map([
      [
        role,
        state.movable.dests.get((util.letterOf(role, true) + "@") as cg.Orig)!,
      ],
    ]);
...
export function renderPockets(state: State): void {
  function renderPocket(pocketEl?: HTMLElement) {
    let el: cg.PieceNode | undefined = pocketEl?.firstChild as
      | cg.PieceNode
      | undefined;
    while (el) {
...
function pocket2str(pocket: cg.Pocket) {
  ...
  for (const role in pocket) {
    letters.push(
      util.letterOf(role as cg.Role, true).repeat(pocket[role as cg.Role] || 0)
    );
  }
  return letters.join("");
}
...
export function pockets2str(pockets: cg.Pockets): string {
  return (
    "[" +
    pocket2str(pockets["white"]!) +
    pocket2str(pockets["black"]!).toLowerCase() +
    "]"
  );
}
