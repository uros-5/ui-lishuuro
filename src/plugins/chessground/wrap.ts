...
export function renderWrap(
  ...
  if (isMiniBoard(element)) {
    if (s.pockets) {
      ...
      container.insertBefore(
        s.orientation === "white" ? pocketTop : pocketBottom,
        board
      );
      container.insertBefore(
        s.orientation === "white" ? pocketBottom : pocketTop,
        board.nextSibling
      );
    }
  }
  ...
  if (s.coordinates) {
    coordFormat[s.notation].forEach((f) => {
      const max =
        f.position === "side" ? s.dimensions.height : s.dimensions.width;
      const coords = f.coords.slice(0, max);
      container.appendChild(
        renderCoords(
          coords,
          `${f.position} ${f.direction}${
            f.noBlackReverse ? "" : " " + s.orientation
          }`
        )
      );
    });
