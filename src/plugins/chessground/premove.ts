...
function kingShako(
  ...
  return (x1, y1, x2, y2) =>
    ...
    (canCastle &&
      ...
      y1 === (color === "white" ? 1 : 8) &&
      x1 === 5 &&
      ((x2 === 7 && rookFiles.includes(8)) ||
        (x2 === 3 && rookFiles.includes(1))));
}
...
const amazon: Mobility = (x1, y1, x2, y2) => {
  return (
    bishop(x1, y1, x2, y2) || rook(x1, y1, x2, y2) || knight(x1, y1, x2, y2)
  );
};
...
function shogiSilver(color: cg.Color): Mobility {
  return (x1, y1, x2, y2) =>
    ferz(x1, y1, x2, y2) ||
    (x1 === x2 && (color === "white" ? y2 === y1 + 1 : y2 === y1 - 1));
}
...
function xiangqiElephant(color: cg.Color): Mobility {
  return (x1, y1, x2, y2) =>
    elephant(x1, y1, x2, y2) && (color === "white" ? y2 < 5 : y2 > 4);
}
...
function janggiPawn(color: cg.Color, geom: cg.Geometry): Mobility {
  ...
  return (x1, y1, x2, y2) => {
    const palacePos = oppPalace.findIndex(
    ...
    return (
      minixiangqiPawn(color)(x1, y1, x2, y2) ||
      additionalMobility(x1, y1, x2, y2)
    );
  };
}
...
function janggiRook(geom: cg.Geometry): Mobility {
  ...
  return (x1, y1, x2, y2) => {
    let additionalMobility: Mobility;
    const wPalacePos = wPalace.findIndex(
      (point) => point[0] === x1 && point[1] === y1
    );
    const bPalacePos = bPalace.findIndex(
      (point) => point[0] === x1 && point[1] === y1
    );
    ...
    switch (palacePos) {
      case 0:
        additionalMobility = (x1, y1, x2, y2) =>
          xd === yd && x2 > x1 && x2 <= x1 + 2 && y2 < y1 && y2 >= y1 - 2;
        break;
      case 2:
        additionalMobility = (x1, y1, x2, y2) =>
          xd === yd && x2 < x1 && x2 >= x1 - 2 && y2 < y1 && y2 >= y1 - 2;
...
function janggiKing(color: cg.Color, geom: cg.Geometry): Mobility {
  const palace = palaces[geom]![color];
  return (x1, y1, x2, y2) => {
    const palacePos = palace.findIndex(
      (point) => point[0] === x1 && point[1] === y1
    );
    let additionalMobility: Mobility;
    switch (palacePos) {
...
const musketeerUnicorn: Mobility = (x1, y1, x2, y2) => {
  ...
  const yd = diff(y1, y2);
  return (
    knight(x1, y1, x2, y2) || (xd === 1 && yd === 3) || (xd === 3 && yd === 1)
  );
};
// musketeer dragon
...
const musketeerFortress: Mobility = (x1, y1, x2, y2) => {
  ...
  const yd = diff(y1, y2);
  return (
    (xd === yd && xd < 4) || (yd === 0 && xd === 2) || (yd === 2 && xd < 2)
  );
};
// musketeer spider
...
function chakWarrior(color: cg.Color): Mobility {
  return (x1, y1, x2, y2) =>
    toriCrane(x1, y1, x2, y2) && (color === "white" ? y2 >= 4 : y2 <= 4);
}
